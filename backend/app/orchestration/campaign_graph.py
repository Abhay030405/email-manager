...
def _safe_goal(raw_goal: str | None) -> str:
...
def _fallback_parse_brief(state: CampaignState) -> CampaignState:
	...
	words = [word for word in brief.split() if word]
	next_state["parsed_data"] = {
		...
		"key_messages": words[:3] if words else ["Discover our latest offering"],
		"constraints": None,
	}
	return next_state
...
async def strategy_node(state: CampaignState) -> CampaignState:
	...
	async def _op(current_state: CampaignState) -> CampaignState:
		...
		parsed_data["campaign_goal"] = _safe_goal(parsed_data.get("campaign_goal"))
